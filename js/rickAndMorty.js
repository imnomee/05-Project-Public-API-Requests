...
function createCards(data) {
    ...
    data.forEach(user => {
        ...
        <div class="card-info-container">
            ...
            <p class="card-text cap"><a href="${user.location.url}">${user.location.name}</a></p>
        </div>
    </div>`;
    });
...
function showModal(index) {
    ...
    console.log(userID);
    const img = userID.image;
    const name = userID.name;
    const status = userID.status;
    const species = userID.species;
    const address = `<a href="${userID.location.url}">${userID.location.name}</a> `;
    const rawDOB = new Date(userID.created);
    const created = rawDOB.toLocaleDateString();
    ...
    <div class="modal-info-container">
        ...
        <p class="modal-text">${status}</p>
        <p class="modal-text">${species}</p>
        
        <hr>
        <p class="modal-text">${address}</p>
        <p class="modal-text">Birthday: ${created}</p>
    </div>
</div>
